...
async def hit(enc_uid, region, token, session):
    try:
        ...
        async with session.post(url, data=bytes.fromhex(enc_uid), headers=headers, timeout=5, ssl=False) as r:
            if r.status != 200:
                return None
            raw = await r.read()
...
def visit():
    ...
    if usage["count"] >= DAILY_LIMIT:
        return jsonify({"error": "Daily limit exceeded", "RequestsToday": f"{usage['count']}/{DAILY_LIMIT}"}), 429
